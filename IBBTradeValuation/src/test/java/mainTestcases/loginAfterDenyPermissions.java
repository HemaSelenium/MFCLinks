...
import Pages.Exterior;
import Pages.Interior;
import Pages.TestDrive;
import Pages.UnderBody;
import Pages.UnderTheHood;
import Pages.VehicleFeatures;
 ...
 *
 */
public class loginAfterDenyPermissions {
	public static AndroidDriver<MobileElement> driver ;
	protected static AppiumHelper helper = new AppiumHelper();
	protected static Documents document=new Documents();
	protected static BasicDetails basicDetails=new BasicDetails();
	protected static Exterior Exteriors=new Exterior();
	protected static UnderTheHood UndertheHood=new UnderTheHood();
	protected static TestDrive testdrive =new TestDrive();
	protected static UnderBody underbody=new UnderBody();
	protected static Interior interiors=new Interior();
	protected static VehicleFeatures vehfeatures=new VehicleFeatures();
	
	
	public static String Username="valuator_reg";
	public static String Password="valuator_reg";
...
caps.setCapability(MobileCapabilityType.PLATFORM_VERSION, "8.0.0");*/
	...
	//	caps.setCapability("autoGrantPermissions",true);
	   ...
	   public void login() throws Exception {
		 ...
		 System.out.println("Avialble permissions in App Are: ");
		 
				 while(driver.findElementByXPath(HomePage.Deny).isDisplayed()) {
				
				driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
				System.out.println(AppiumHelper.FindElementById(driver, "com.android.packageinstaller:id/permission_message").getText());
				helper.clickOnElementByXpath(driver, HomePage.Deny);
				driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
				//throw new Exception("");	
		  ...
		  if (AppiumHelper.FindElementByXpath(driver, HomePage.Home).isDisplayed()) {
				System.out.println("After denyed the permissions, its logged in successfully and navigated to Home page");

			} else {
				System.out.println("After denyed the permissions, page is not navigated to home page.");
			}
			
		  /* */
	   }
	   catch(NoSuchElementException e) {
		   System.out.println("After Deny the permissions, unable to login");
	   }
	   }
@AfterClass
