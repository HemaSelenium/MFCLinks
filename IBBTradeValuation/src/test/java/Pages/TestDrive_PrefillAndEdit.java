 ...
 *
 */
public class TestDrive_PrefillAndEdit {
	...
	public void Engine(AndroidDriver<MobileElement> driver, String engine) throws Exception {
			...
			helper.selectMulti_aftersubmit(driver, engine);
			try {
			if (AppiumHelper.FindElementByXpath(driver, Engine_tick).isDisplayed()) {
				System.out.println("After Edited Engine, Tick mark  displayed");
			}
		} catch (NoSuchElementException e) {
			System.out.println("After Edited Engine, Tick mark didnt display");
		}
	}
public void clutchSystem(AndroidDriver<MobileElement> driver, String clutchsystem) throws Exception {
		...
		helper.selectMulti_aftersubmit(driver, clutchsystem);
	   
		try {
		if (AppiumHelper.FindElementByXpath(driver, clutchSystem_tick).isDisplayed()) {
			System.out.println("After Edited clutchSystem, Tick mark  displayed");
		}
	} catch (NoSuchElementException e) {
		System.out.println("After Edited clutchSystem, Tick mark didnt display");
	}
	}
public void Transmission(AndroidDriver<MobileElement> driver, String Transmission) throws Exception {
	...
	helper.selectMulti_aftersubmit(driver, Transmission);

	try {
		if (AppiumHelper.FindElementByXpath(driver, transmission_tick).isDisplayed()) {
			System.out.println("After Edited transmission, Tick mark  displayed");
		}
	} catch (NoSuchElementException e) {
		System.out.println("After Edited transmission, Tick mark didnt display");
	}
}
